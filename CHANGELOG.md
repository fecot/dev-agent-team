...
### Added
- **実運用 Insights レポート（第 2 弾）由来の 4 点取り込み** (前回レポートの対象期間を 2 週間延長した続報。事故の大半は前回の 5 点で対応済みのため、取りこぼしのみ) — (1) **ステージング規律** — ステージは明示的に扱ったファイルだけ。`git add -A` / `--renormalize` の一括ステージで意図しないファイルが混入した実例から（`workflows` Phase 5 Action / `commands/safe-implement.md` 原則）、(2) **最初のコミット前の作業ブランチ確認を〔機械検証可〕Stop Condition 化** — コマンド連鎖の途中失敗で main 上のまま作業していた実例から。§2.7 の hooks 化例に PreToolUse での差し戻しを追加（「言葉より仕組み」の適用例）、(3) **レビュー指摘の確信度タグ** — 〔確認済み〕/〔推定〕/〔未検証〕を付け、未検証の懸念は必須 / 推奨修正に混ぜず「未検証の仮説」に分離（`agents/review-gatekeeper.md` 出力テンプレ + 行動原則、`commands/pr-review.md`）。前回の調査レポート「未検証」マークとレビュー成果物側の語彙を揃えた、(4) **バグ修正の再現テストは fail-first を実証** — 修正前コミットで実際に落ちることを確認してから修正後に通す。「修正後に通る」だけではバグを捉えている証明にならない（`skills/test-design.md` 優先順位 / `workflows` Phase 6 Action + Stop Condition）。同種のバグが QA 指摘として反復していた実例から。**見送り**: バグ調査プロトコル（実測 → 仮説 → 人間が選ぶまで編集しない）は「コードを読む前に仮説を立てない」+ Phase 4 計画承認で既出、5 ファイル超の事前計画は Phase 4 の方が強い、その他（persona レビュー常設化 / ドキュメントテンプレ化 / Bash 列のスクリプト化 / MCP 連携）は既存構造の追認またはユーザー固有
