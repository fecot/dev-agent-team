...
この関係は「置き換え」ではなく **積層** です。キットの構成要素を棚卸しすると、ネイティブ機能に丸ごと置き換えられたものはなく、多くは **ネイティブ機能を包んで適用範囲とガードレールを付加する** 形になっています（`/goal` は機械的サブループ限定、Scheduled Tasks は自己再発火の禁止、等）。一方で、汎用的な行動指針はモデル世代が上がるたびにデフォルト挙動へ吸収されていきます。だからキットの価値の重心は、**現場固有の蓄積** — Known Risks / Decision Log / Migration チェックリストのような「使うほど・失敗するほど太る台帳」— の側へ移動し続けます。詳細は [`CONCEPT.md`](CONCEPT.md)「型とは何か」。
