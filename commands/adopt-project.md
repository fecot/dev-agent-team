...
分岐 E（バージョン不足）専用。**強制続行は提供しない**。選択肢の内容は §分岐 E を正とする（ここには複製しない）。
...
| ランク | アイコン | 意味 | Phase 0 での扱い |
|---|---|---|---|
| **必須** | ⚠️ | 段階1で答えるべき項目 | 必ず Stop Condition |
| **依頼依存** | 🔶 | 依頼内容に関連する場合のみ問題になる | 該当依頼時のみ Stop Condition |
| **推奨** | ℹ️ | 任意項目 | Stop Condition にはならないが、判断精度が上がる |
...
重要度ランクとアイコンの対応は §未確定項目の重要度ランク を正とする。メッセージ文言は上の出力例を正とする。
