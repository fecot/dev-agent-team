...
- **質問には回答のみ返す**。「〜すればいいだけだよね？」への正しい応答は Yes / No と根拠であって、実装やリソース作成ではない（判断材料を出した時点で人間に返す — `docs/native-tooling-integration.md` §8.1 の assessment 境界）
- **リモート状態を変える操作（push / PR 作成・更新 / Secret 登録 / タグ作成）は明示承認を得てから行う**。コミットまではローカルで自由。追加コミットの push にも改めて承認を得る
...
- セキュリティ・個人情報の取り扱い
- PR ルール / Release ルール
- **Environment Notes**（環境の恒久的な制約: ホスト制約 / 権限的に触れないパス / 外部連携の認証状態）— 以降の Phase の前提にする
- **Decision Log**（過去の案件で人間が下した判断と理由）— 今回の依頼に関係する記録があれば、その前提から始める。記録どうしが衝突する / 前提が噛み合わない場合は AI が採用を決めず人間に戻す
