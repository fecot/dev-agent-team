...
2. `git branch --show-current` で作業ブランチにいることを確認してから、ImplementationDriver として実装を開始（main / master 上なら止まる）
...
- 1PRに含める変更を最小化する
- コミットは意味のある単位で細かく切る
- ステージは明示的に扱ったファイルだけ（`git add -A` / `--renormalize` を既定にしない。一括ステージするなら差分を見せてから）
