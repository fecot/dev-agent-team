...
**dev-agent-team の立ち位置**: 本キットが効くのは **Tier 3** だ。`/goal`・Dynamic Workflows・Scheduled routine はどれも Tier 3（AI 起点）のエンジンで、放っておくと「AI が先に動く」勢いのまま **人間の確認ゲートも飛び越えかねない**。dev-agent-team は Tier 3 に **「AI が先に動いてよい区間」と「必ず人間が確認する区間（Human Decision Point）」の線引き** を持ち込む。つまり本キットは、**確認ゲート付きで Tier 3 を安全に使うための型**である。§2（`/goal`）・§3（Dynamic Workflows）・§6（Agent Teams）・§7（Scheduled Tasks）の併用ルールは、この線引きを機能別に具体化したものだ。

> Tier 3 のリスクは「確認ゲートを飛び越える」だけではない。**ゲートの手前で止まらなくなる**（人間の応答待ちを自動ループが監視し続ける）事故も同じ Tier 3 で起きる。特に Scheduled Tasks は **セッションの外に発火点を持つ** ため、暴走しても気付きにくい。→ §7
...
1. **指示が自己完結** — 対象ファイル・変更内容・従うべき規約・「〜以外は触らない」まで明記し、**why（この変更が何のためか）を 1 行添える**（§8.3 理由の伝達）
2. **判断余地を残さない** — 「適切に」「いい感じに」を含めない。low は最も安全な解釈（最小限の変更）に倒れる
3. **機械的検証をセットで渡す** — `bash -n` / `node --check` / grep 確認を指示に含める
4. **報告義務を課す** — 「迷って変更しなかった箇所を理由付きで報告」させる（黙って省略させない）。報告の各主張は **ツール結果と突き合わせてから** 書かせる（§8.3 証拠つき報告）
...
| 定期的な外部状態の観測（依存の更新 / CI の傾向など） | Scheduled Tasks（§7）。**終了条件を自身の出力で判定できる形にし、新しい発火点を増やさない** |
| 人間の応答待ち（マージ待ち / 承認待ち）を追いかけたい | **監視しない。人間に返す**（§7.1）。終了条件が人間の行動に依存するループは、原理的に終わらない |
...
**人間ゲートは「越えさせない」だけでは足りず、「待たせてもいけない」** — ゲートの手前に自動ループを置くと、人間が動くまで状態が変わらないため終了条件が存在しなくなります（§7.1）。ゲートに達したら、エンジンは**止まるのではなく終了する**のが正しい振る舞いです。
...
> キットの `agents/*.md` をネイティブ subagent 定義化すればチームメイト定義として再利用できます（公式 docs "Use subagent definitions for teammates"）。これは §9 Follow-up の subagent 定義化と同一の意思決定です。

---

## 7. Scheduled Tasks / ルーチンの併用ルール（自己再発火の禁止）

Scheduled Tasks（定期実行ルーチン / cron 実行のクラウドエージェント）は、**人間が起動しなくても AI が動き出す**唯一のエンジンです。`/goal` はセッション内、Dynamic Workflows は 1 回の実行内で完結しますが、ルーチンは **セッションの外に発火点を持ちます**。ここでの失敗は他のエンジンと形が違い、**人間ゲートを越えるのではなく、人間ゲートの前で止まらなくなる** 形で現れます。

### 7.1 人間ゲートは「越えさせない」だけでなく「待たせない」

§5 の原則は「エンジンに人間ゲートを越えさせない」ですが、ルーチンではその裏返しの事故が起きます:

> **人間の応答待ちを、自動ループの監視対象にしてはいけない。**

PR のマージ・受入基準の承認・仕様確認への返信は、**人間が動くまで状態が変わりません**。そこに「変化がなければ再チェックする」ループを掛けると、**終了条件が原理的に存在しない**ループが完成します。ゲートを越えていないので安全に見えますが、外部から止まる要因がないぶん、越える事故より止まりにくい。

- **禁止**: 「PR がマージされるまで N 分ごとに確認」「承認が来るまで再チェック」型のルーチン
- **正**: PR を作ったらそこで終了する。**その後を見る責任は人間にある**（Human Decision Point の定義そのもの）
- 状況を知りたいなら、AI に監視させるのではなく **人間側の通知**（GitHub の通知 / レビュー依頼）を使う

### 7.2 自己再発火の禁止 — 4 層の歯止め（強度順）

ルーチンが「将来の自分」を予約できると、1 回の判断ミスが無限に増殖します。**個別にはまっとうな判断でも、終了条件を欠くと暴走になります**。歯止めは §5.2 の 3 層と同じ思想で、**言葉より仕組みが強い**:

| 強度 | 層 | 実体 |
|---|---|---|
| ①最強 | **能力を与えない** | 予約・トリガー作成系のツール / コネクタをそのルーチンに渡さない（least-privilege。`disallowed-tools` と同じ発想 → §9） |
| ② | **回数を持たせる** | 再実行に「あと N 回」の残数を持たせ、必ず減らす（`/goal` の `or stop after N turns` と同じ役割 → §2.3） |
| ③ | **痕跡を必ず残させる** | 実行のたびに最小限の報告を残させる（Issue コメント等）。無音の実行を作らない |
| ④最弱 | **言葉で禁じる** | プロンプトのルールとして書く。**単独では歯止めにならない**（③②①と併用する） |
...
- 将来の自分を予約しないこと。send_later、自己チェックイン、
  ...
  PR を作った後の進捗監視もしない。
- PR のその後は人間が見る。あなたは気にしなくてよい。
- 仕事が終わったら、その場でセッションを終了する。
```

> 「自己再発火を禁止した」と宣言しても、**塞いだのが外部イベント駆動の再発火だけ** なら自己チェックインは残ります。禁止対象は「新しい発火点を作る行為すべて」として書くこと。

### 7.3 新しい発火点を増やさない

コストの上限は、プロンプトではなく **発火点の数** で構造的に決まります。

- 追加の監視・確認が必要になったら、新しいルーチンを作らず **既存の定期実行に相乗り** させる（発火点が増えないので、実行回数の上限が構造的に固定される）
- **終了条件が人間の行動に依存するなら、それは終了条件ではない**。ルーチンの終了条件は、そのルーチン自身の出力かツール結果で判定できる形にする（§2.3 の `/goal` 条件と同じ制約）
- **人間側の処理速度も系の一部**として設計する。「人間が数日見ない」状態を前提に、その間に何回実行されるかを見積もる

### 7.4 静かな失敗を作らない

**通知抑制と自己再発火の組み合わせが、検出を最も遅らせます**。「変化がなければ通知不要」は一見親切ですが、変化がないときこそループが続いている状態なので、異常が可視化されません。

- 実行のたびに痕跡を残す（7.2 ③）。「起動に失敗したら理由を Issue にコメントする。黙って終わるな」まで指示に含める
- §8.3 の「証拠つき報告」と対になる原則: **報告は証拠に基づかせる。同時に、報告そのものを省略させない**

### 7.5 暴走の検出方法

トークン使用量のグラフには「誰が使ったか」が出ないため、**使用量からは特定できません**。

- **兆候**: 依頼を投げていない日にトークンが減っている
- **確認場所**: ルーチン管理画面の一覧を **「完了済みを含める」フィルタを有効にして** 見る（実行済みは既定で隠れる）
- **判定**: 身に覚えのない名前が一定間隔で並んでいたら、それが原因

> 定期実行を運用するプロジェクトでは、この確認手順を Project Rules の Known Risks / 運用手順に書いておく（`templates/project-rules-template.md` の「Execution Engine 方針」）。

---

## 8. Fable 5 世代のプロンプト設計 — 公式ガイドとの整合

Anthropic 公式の [Claude Fable 5 プロンプティングガイド](https://platform.claude.com/docs/ja/build-with-claude/prompt-engineering/prompting-claude-fable-5) は、Fable 5 / Mythos 5 世代での挙動変化と推奨プロンプトパターンを定義しています。本節はキットとの対応を整理します。結論から言うと **公式パターンの多くは、キットが既に「型」として持っているものと一致します**（答え合わせ）。一方で、世代適合のために新しく取り込む指針もあります。

### 8.1 公式パターンがキットの型を裏打ちする
...
この一致は偶然ではありません。「どこで止まるか」を明示する型は、モデルの自律性が上がるほど価値が増します（§1.1 エージェンシー・ラダーの帰結）。

### 8.2 指示粒度の世代適合 — ゲートは列挙、行動は簡潔

公式ガイドは「**従来のモデル向けに開発されたスキルは、Fable 5 には指示が細かすぎることが多く、出力品質を低下させる可能性がある**」と明言しています。指示追従が強い世代では、簡潔な原則 1 つが細かい列挙と同等以上に効きます。キットに適用する際の線引き:

- **維持（列挙のまま）**: Stop Condition / Human Decision Point / 禁止事項。これらは「どこで止まるか」の契約（判断の型）であって行動のマイクロ指示ではなく、列挙自体が価値
- **簡潔化候補**: 行動手順の細かい列挙（「まず X し、次に Y し…」型）。Fable 5 世代では **原則 + 意図（why）** の記述に置き換えた方が品質が上がりうる

agents/ / skills/ を見直す際（§9 Follow-up のネイティブ化を含む）は、この基準で監査します。

### 8.3 委任・長時間実行のプロンプトパターン

司令塔 → 実装エージェントの委任（§3.6）や、長時間フェーズの実行で有効な公式パターン:

- **証拠つき報告** — 進捗・完了報告の各主張を、**そのセッションのツール結果と突き合わせてから** 書かせる（"Before reporting progress, audit each claim against a tool result"）。公式テストでは、捏造を誘発するよう設計されたタスクでも捏造ステータスレポートをほぼ排除。§3.6 成立条件 4（報告義務）の具体形
- **理由の伝達** — 委任時は what（対象・変更内容）だけでなく **why（この変更が何のためか・誰のためか）を 1 行添える**。意図が分かるとタスクを関連情報に正しく結びつけられ、意図の推測で外すことが減る。§3.6 成立条件 1（自己完結指示）に含める
- **interval 検証** — 長時間の実装では「**一定間隔で、新しいコンテキストの検証サブエージェントに仕様と突き合わせさせる**」工程をプロンプトに明示する（"Run this every [X interval], verifying your work with subagents against the specification"）。Phase 5 の長丁場や Migration で有効
- **長寿命サブエージェント + 非同期** — サブタスク間でコンテキストを保持する長寿命サブエージェントはキャッシュ読み取りで時間とコストを節約する。各サブエージェントの完了をブロックして待つより、非同期に進捗を受ける構成を優先する

### 8.4 禁止 — 内部推論をエコーさせる指示

Fable 5 は「モデルの内部推論を応答テキストとして書き起こさせる指示」を `reasoning_extraction` の refusal カテゴリとして扱います。スキル・テンプレート・エージェント定義に **「思考過程を説明せよ」「検討の内部過程を出力せよ」系の指示を書かない** こと:

- 成果物としての比較・根拠（実装案 A/B/C の比較、採用理由の記録、レビュー指摘の理由）は **問題ない** — これは deliverable であり、内部推論のエコーではない
- 必要なのは「推論の可視化」ではなく「判断材料の提示」。キットの既存文書は監査済みで該当なし（2026-07 時点）。今後スキル・テンプレートを追記する際にこの線を守る
