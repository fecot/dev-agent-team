...
### 1.2 ネイティブ機能の取り込み基準 — ゲートに触れる変更だけを追う

Claude Code は速いペースで更新される。全変更を追随して本ドキュメントに反映し続けると、**キットの維持コストがベンダーのリリース速度に比例して増え、上限がなくなる**。これは §7.3 で戒めた「発火点を増やす」のと同型の消耗であり、同じ処方——追随の入口を絞って上限を構造的に決める——を自分に適用する。

取り込むのは次の 1 条件を満たす変更だけ:

> **人間ゲートの位置または強度が変わる変更か？**
> — 新機能・仕様変更によって、これまで人間が判断していた箇所を AI が越えられるようになる／逆にランタイムが新たに守ってくれるようになる場合のみ、本ドキュメントに反映する。

具体的には:

- **取り込む**: 承認フローの変更、自律実行機能の追加（新しい Tier 3 エンジン）、permissions / auto mode の挙動変更、エージェント間承認の仕様変更 — つまり §2 / §3 / §5 / §6 / §7 の前提を変えるもの
- **取り込まない**: 性能改善、UI 変更、新しい便利機能（ゲートに触れないもの）、モデルの世代交代そのもの — 使えば分かるものは文書化しない
- **バージョンピンの新規追加も同基準**。冒頭の前提バージョン一覧は「ゲートに関わる機能の登場時期」の記録であり、網羅的な互換表ではない

> ランタイムがゲートを吸収した場合（§5.1 のような「キット側の変更不要」事例）は、注記を 1 行足すに留める。ランタイムに吸収された防御をキット側で二重に書き続けることはしない——文書の役割は「どこが二重防御か」を示すことであって、防御を複製することではない。
...
> **監査済み（2026-08）**: agents/ 7 本・skills/ 8 本をこの基準で棚卸しした結果、いずれも既に「原則 + why」の粒度に収まっており、大きな削減対象は無かった（マイクロ指示の集積は `commands/adopt-project.md` に集中しており、重複・開発メモの削除で対応済み）。以後の見直しは、ファイルを新規追加・大幅改稿したときにこの基準を通せば足りる——定期的な全量再監査はしない（§1.2 と同じく、維持コストに上限を設ける）。
