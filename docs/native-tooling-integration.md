...
1. **指示が自己完結** — 対象ファイル・変更内容・従うべき規約・「〜以外は触らない」まで明記し、**why（この変更が何のためか）を 1 行添える**（§7.3 理由の伝達）
2. **判断余地を残さない** — 「適切に」「いい感じに」を含めない。low は最も安全な解釈（最小限の変更）に倒れる
3. **機械的検証をセットで渡す** — `bash -n` / `node --check` / grep 確認を指示に含める
4. **報告義務を課す** — 「迷って変更しなかった箇所を理由付きで報告」させる（黙って省略させない）。報告の各主張は **ツール結果と突き合わせてから** 書かせる（§7.3 証拠つき報告）
...
> キットの `agents/*.md` をネイティブ subagent 定義化すればチームメイト定義として再利用できます（公式 docs "Use subagent definitions for teammates"）。これは §8 Follow-up の subagent 定義化と同一の意思決定です。

---

## 7. Fable 5 世代のプロンプト設計 — 公式ガイドとの整合
...
agents/ / skills/ を見直す際（§8 Follow-up のネイティブ化を含む）は、この基準で監査します。

### 7.3 委任・長時間実行のプロンプトパターン
