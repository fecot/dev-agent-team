...
### 0. 全画面スクリーンショット + 可視要素インベントリ

個別要素の計測に入る前に、**まず画面全体を実機スクショで撮り、見えている要素を棚卸しする**。コードや要件から拾った要素だけを計測すると、「画面に存在するのに認識していなかった要素」を丸ごと見落とす（例: 一覧の補助グラフ、状態バッジ、フッタ、空状態の差し込み等）。

- source 画面の **フルページスクリーンショット**（スクロール領域含む）
- 主要状態ごとのスクショ（空 / データあり / エラー / hover など、再現に関わる状態）
- スクショから **可視要素を列挙したインベントリ**（要素名 / 役割 / 後続の計測対象か）
- このインベントリを Phase 4 の再現対象リストの起点にする（「コードに無かったから作らなかった」を防ぐ）
...
- 全画面スクショ + 可視要素インベントリを取らず、コード由来の要素だけ計測して Phase 4 に進もうとしている（要素の丸ごと見落としリスク）
