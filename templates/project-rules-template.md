...
### Environment Notes（環境の恒久的な制約）

- {{ホスト環境の制約（例: ホストの Node が古いため、テスト・dev server は必ず Docker / pnpm script 経由で実行）}}
- {{権限的に読み書きできないファイル・パス（例: `.env` は編集不可。必要な行はそのまま出力し、人間が貼る）}}
- {{外部サービス連携の認証状態（例: チケット管理は OAuth 未設定で API 到達不可 / スプレッドシートは書き込み不可、TSV で渡す）}}

> タスクごとに再発見するものではなく、このマシン・このリポジトリの **安定した事実** をここに書く。環境・権限のブロックは「作業がほぼ完成した最終盤」に発覚しがちなので、Phase 0 の読み込みで前倒しする。ブロックに新しく当たったら 1 行追記する（Known Risks と同じ蓄積運用）。
...
- **Scheduled Tasks / 定期実行ルーチンの使用**: {{許可 / 禁止 / 外部状態の観測のみ / 該当なし}}
  - **自己再発火（将来の自分の予約 / 再アーム）**: 原則 **禁止**。許可する場合は {{回数上限（あと N 回）と、実行ごとに痕跡を残す先}} を明記する
  - **人間の応答待ちの自動監視は禁止**（PR のマージ待ち / 承認待ちを定期チェックするルーチンを作らない。終了条件が存在しないため）
  - **暴走の確認手順**: 依頼を投げていない日にトークンが減っていたら、ルーチン一覧を「完了済みを含める」で確認する
- **人間ゲートの扱い**: 承認・意思決定（受入基準 / 採用案 / マージ / リリース / DB・権限変更）はエンジンに委譲せず必ず人間が判断する。ゲートに達したエンジンは **待機せず終了** する
...
- インフラ構成の変更
- 本番データの直接操作
- **リモートリポジトリの状態変更**（`git push` / PR 作成・更新 / Secret 登録 / タグ作成）— コミットまではローカルで自由。リモートを変える操作は明示承認。**最初の push の承認は、その後の全 push の承認ではない**
...
過去の案件で **人間が下した判断** と、その理由を残す。同じ問いが次に来たとき、AI がここを読んで前回と同じ前提から始められるようにする。

| 判断 | 理由 | 再利用条件 |
|---|---|---|
| {{何を決めたか（1行）}} | {{なぜそう決めたか}} | {{この判断が通用する範囲。「この案件のみ」/「同じスタックなら」/「プロジェクトを問わず」}} |

> このセクションは Known Risks と同じく **振り返りで育てる**。Phase 8（Release Check）の振り返りで「次も同じ問いが来る」判断を抽出し、人間の承認のうえ 1 行ずつ追記していく（追記提案は AI が出すが、書き込みの可否は人間が判断する）。
> **理由を省略しない** — 判断だけが残ると、前提が変わったときに見直せず、ただの縛りになる。理由は「再判断するための手がかり」として書く。
> **再利用条件が「プロジェクトを問わず」の行は、案件を横断する原則の候補**。対象リポジトリの外（全社の開発規約 / 個人の `~/.claude/CLAUDE.md` 等）へ引き上げるかは人間が判断する。
> 一度きりの判断（この案件でしか出てこない選択）は書かない（ノイズになる）。
